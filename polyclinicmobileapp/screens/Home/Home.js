...
const Home = ({ navigation }) => {
  ...
  const menuItems = [
    ...
    { key: 'manageMedicine', label: 'Quản lý thuốc', icon: 'flask' },
  ...
  const handleMenuPress = key => {
    ...
    if (key === 'prescription') navigation.navigate('MyAppointment');
    if (key === 'manageMedicine') navigation.navigate('MedicineManagement');
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.header}>
        ...
        <Searchbar placeholder="Search" onChangeText={setSearchQuery} value={searchQuery} style={styles.searchBar} inputStyle={styles.searchInput} />
      </View>
